...
export function ConsumerGroupDetail({ groupId, onBack }: Props) {
  ...
  const memberColumns = [
    { key: "clientId", label: "Client ID", render: (r: Record<string, unknown>) => (
      <span className="font-mono text-amber-300">{String(r.clientId)}</span>
    )},
    { key: "clientHost", label: "Host" },
    { key: "partitions", label: "Partitions", render: (r: Record<string, unknown>) => {
      const parts = r.partitions as string[];
      return (
        <div className="flex gap-1 flex-wrap">
          {parts.slice(0, 5).map((p) => (
            <span key={p} className="text-[9px] bg-slate-800 text-slate-300 rounded px-1.5 py-0.5 font-mono">{p}</span>
          ))}
  ...
  const offsetColumns = [
    { key: "topic", label: "Topic", render: (r: Record<string, unknown>) => (
      <span className="font-mono text-indigo-300">{String(r.topic)}</span>
    )},
    ...
    { key: "currentOffset", label: "Current Offset", className: "w-36 text-right", render: (r: Record<string, unknown>) => (
      <span className="font-mono">{Number(r.currentOffset).toLocaleString()}</span>
    )},
    ...
    { key: "lag", label: "Lag", className: "w-28 text-right", render: (r: Record<string, unknown>) => {
      ...
      return <span className={`font-mono font-bold ${color}`}>{fmt(lag)}</span>;
    }},
  ];
  ...
  // Get unique topics for reset dropdown
        ...
        {selectedConsumerGroup && (
          <div className="flex items-center gap-3">
            <span className={`text-xs font-medium px-2 py-1 rounded-lg ${
              selectedConsumerGroup.state === "Stable" ? "bg-emerald-500/15 text-emerald-400" : "bg-slate-800 text-slate-400"
            }`}>
              {selectedConsumerGroup.state}
            </span>
            <button
              onClick={() => setShowReset(true)}
              className="px-3 py-1.5 rounded-xl text-xs font-medium bg-amber-500/20 border border-amber-500/40 text-amber-300 hover:bg-amber-500/30 transition-all cursor-pointer"
            >
              Reset Offsets
            </button>
          </div>
      ...
      <Modal title="Reset Consumer Group Offsets" open={showReset} onClose={() => { setShowReset(false); setResetResult(null); }}>
        <div className="space-y-4">
          ...
          <p className="text-sm text-slate-300">
            Reset offsets for <span className="font-mono text-amber-300">{groupId}</span>. The consumer group must be inactive (no running consumers).
          </p>
          <div>
          ...
          <div>
            ...
            <select
              ...
              className="w-full mt-1 bg-slate-800/80 rounded-xl px-3 py-2 border border-slate-700/50 text-sm text-white focus:outline-none focus:border-amber-500/50"
            >
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <button
              onClick={handleReset}
              disabled={resetting}
              className="px-4 py-2 rounded-xl text-sm font-medium bg-amber-500/20 border border-amber-500/40 text-amber-300 hover:bg-amber-500/30 transition-colors cursor-pointer disabled:opacity-40"
            >
